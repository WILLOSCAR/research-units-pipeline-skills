...
For an exploratory effort spanning several open questions, keep one map plus one child file per
decision ticket:
...
Implementation tickets carry the default `ready-for-agent` status and are not triage inputs.

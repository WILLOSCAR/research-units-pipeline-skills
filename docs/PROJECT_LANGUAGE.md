...
- **Checks the verdict where a scorecard exists.** The scorecard checks
  re-derive the result from the evaluated inputs (`run-evaluation.v1` on the
  legacy path) rather than reading the claimed verdict. The structural report
  checks are weaker and not uniform: several accept a `Status: PASS` line on its
  own. Which applies is fixed by the check, not chosen per Run.

...
製品モデルは `Goal -> Run -> Evidence -> Artifact` で、verify/repair/re-run の Loop で閉じます。harness が各パスを verify するため、Run は収束して初めて信頼されます。以下の workflow は現在の非公開 Recipe 実装で、Run と Unit は内部詳細です。
